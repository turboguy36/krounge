...
//		Log.i("IMAGE LOC", user_id+" / "+ imageFilePath);
		...
		for(File af: arrFile){
//			Log.i("file path", af.getPath());
//			Log.i("file path", af.length()+"");
		}
		if(bitmap == null) {
			// FlieCache 파일 개수 관리
	...
	private void ManageFileCache(String strFolderPath) {
		...
		int count = 0;
		int fileNum = new File(strFolderPath).listFiles().length;
...
//		Log.i("stored IMG path", strFilePath);
		File file = new File(strFilePath);
		if (file.exists() == false) {
//			Log.i("stored IMG", "null");
			return null;
		}
