...
//	List<GroupMemberInfo> prepared_gmList = null;
	/**
...
//		groupName.setLayoutParams(params);
	...
	void createThreadAndDialog(final List<GroupMemberInfo> groupMemberList) {
		member_group_list = new SparseArray<List<GroupInfo>>();
//		pd = ProgressDialog.show(getActivity(), "", getResources().getText(R.string.msg_contents), true);
		Thread thread = new Thread(new Runnable() {
			public void run() {
				int gmSize = groupMemberList.size();
				KLoungeGroupRequest krgr = new KLoungeGroupRequest();		
				for(int position = 0; position<gmSize; position++){
					List<GroupInfo>group_list =  new ArrayList<GroupInfo>();
					int puser_id = groupMemberList.get(position).getUserId();
					group_list = krgr.getGroupList(AppUser.user_id, puser_id);
					
					member_group_list.append(puser_id, group_list);
				}
				Message msg = Message.obtain();
				msg.obj = member_group_list;
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			SparseArray<List<GroupInfo>> memInfo = (SparseArray<List<GroupInfo>>) msg.obj;
			if(memInfo != null)AppUser.MEMBER_INFO = memInfo;
			// View
			if(gmList.size()==0) {
				...
				if(AppConfig.DEBUG)Log.i(TAG,"Loading Message List Error "+this.getClass().toString());
				getActivity().finish();
			}else{
